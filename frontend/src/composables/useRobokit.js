...
export function resolveOneKey6073Height(form) {
...
export function resolvePickDrop6073Height(form) {
...
export function resolveSpecPath6073Height(form, segList) {
...
export async function handleSpecifiedPathNavigation() {
  ...
  try {
    ...
    if (splitByForkLoad) {
      ...
      try { await api.robokitCancelNavigation() } catch (_) {  }
      try { await api.robokitStopNavigation() } catch (_) {  }
      await sleep(300)
