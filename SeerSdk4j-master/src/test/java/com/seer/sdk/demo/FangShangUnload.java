...
//        double targetHeight = inputStr.trim().isEmpty() ? 0.25 : Double.parseDouble(inputStr);
        ...
        try {
            ...
            // ---------------------------------------------------------
            System.out.println("正在规划路径(1): 从当前位置 到 链条机前置点");

            JSONObject navReqJson1 = new JSONObject();
            navReqJson1.put("id", "LM2");           // 目标点：取货点
            ...
            // ---------------------------------------------------------
            System.out.println("等待到达链条机前置点...");
            waitForTaskCompletion(rbkClient);
            System.out.println("已到达链条机前置点，准备执行下一步。");


            // ---------------------------------------------------------
            // 4. 第二次路径导航 (API 3051) -> 去链条机取货点
            // ---------------------------------------------------------
            System.out.println("正在规划路径(2): 从当前位置 到 链条机取货点");

            JSONObject navReqJson2 = new JSONObject();
            navReqJson2.put("id", "AP1");           // 目标点：送货点
            ...
            // 货叉加载货物，会将叉车状态变成载货中
            navReqJson2.put("operation", "ForkLoad");
            navReqJson2.put("max_speed", 0.15);
            navReqJson2.put("start_height", 0.94);
            ...
            // ---------------------------------------------------------
            // 5. 等待第一个任务完成 (使用 API 1020)
            // ---------------------------------------------------------
            System.out.println("等待到达链条机取货点...");
            waitForTaskCompletion(rbkClient);
            System.out.println("已到达链条机取货点，准备执行下一步。");

            // ---------------------------------------------------------
            // 6. 第三次路径导航 (API 3051) -> 回到前置点调整货叉高度
            // ---------------------------------------------------------
            JSONObject navReqJson3 = new JSONObject();
            navReqJson3.put("id", "LM2");           // 目标点：送货点
            ...
            navReqJson3.put("operation", "ForkHeight");
            navReqJson3.put("max_speed", 0.15);
            navReqJson3.put("start_height", 1.1);
            navReqJson3.put("end_height", 0.25);
            ...
            // ---------------------------------------------------------
            // 6. 第四次路径导航 (API 3051) -> 去放货点
            // ---------------------------------------------------------
            JSONObject navReqJson4 = new JSONObject();
            navReqJson4.put("id", target1Point);           // 目标点：送货点
            ...
            // 货叉卸载货物，会将叉车的状态变成非载货中
            navReqJson4.put("operation", "ForkUnload");
            navReqJson4.put("max_speed", 0.25);
            navReqJson4.put("start_height", 0.25);
            navReqJson4.put("end_height", 0.09);
            navReqJson4.put("max_wspeed", 0.2);

            String navReqStr4 = navReqJson4.toString();
            System.out.println("发送导航请求(3): " + navReqStr4);

            // 使用 API 3051
            RbkResult navResult4 = rbkClient.request(3051, navReqStr4, 5000);

            if (!RbkResultKind.Ok.equals(navResult4.getKind())) {
                System.err.println("导航指令(3)发送失败: " + navResult4.getErrMsg());
                return;
            }
            System.out.println("导航指令(3)发送成功，任务 ID: " + currentTaskId4);
